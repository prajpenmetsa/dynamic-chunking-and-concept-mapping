"""
Slide-based Learning Outcomes (LOs) generation using Bloom's Taxonomy
MAP-REDUCE approach:
  1) For each PDF deck: extract ALL text (text-only; ignores images) and summarize into compact JSON.
  2) Combine all deck summaries and generate 6–7 course LOs using Bloom's Taxonomy.
...
COURSE_CODE = "CS3.301"
OUTPUT_FILE = "/Users/lakshmiprajnapenmetsa/Desktop/iiith/research/irel/honors/dynamic-chunking-and-concept-mapping/datasets/slide_based_los_blooms.json"
...
# ==================== LO GENERATION PROMPT (BLOOM'S TAXONOMY) ====================
def create_lo_generation_prompt(course_title: str, course_code: str, all_deck_summaries_json: str) -> str:
    """
    Final synthesis step: generate course outcomes using Bloom's Taxonomy from all deck summaries.
    """
    return f"""
You are an expert Educational Curriculum Designer.

**Task**: Create 6-7 comprehensive Learning Outcomes (LOs) for this university course based on Bloom's Taxonomy.
...
**Instructions**:
1. Analyze the deck summaries to identify major topics and concepts
2. Generate 6-7 learning outcomes that:
   - Cover the breadth of topics across all decks
...
[
  "CO-1: [Bloom Verb] [specific technical content related to lectures]",
  "CO-2: [Bloom Verb] [specific technical content related to lectures]",
  "CO-3: [Bloom Verb] [specific technical content related to lectures]",
  "CO-4: [Bloom Verb] [specific technical content related to lectures]",
  "CO-5: [Bloom Verb] [specific technical content related to lectures]",
  "CO-6: [Bloom Verb] [specific technical content related to lectures]",
  "CO-7: [Bloom Verb] [specific technical content related to lectures]"
]

**Quality Requirements**:
✓ Each LO must start with a Bloom's Taxonomy verb
✓ Be specific to the technical content from the lecture decks
✓ Progress from lower-order to higher-order thinking skills
...
def main():
    print("=" * 70)
    print("  BLOOM'S TAXONOMY: SLIDE-BASED LO GENERATION (ALL PDFS)")
    ...
    print("\n📝 Generating final Learning Outcomes using Bloom's Taxonomy...")
...
"""
    ...
    final_prompt = create_lo_generation_prompt(COURSE_TITLE, COURSE_CODE, all_summaries_json)
    ...
    print("\n" + "=" * 70)
    print("  GENERATED LEARNING OUTCOMES (Bloom's Taxonomy)")
    print("=" * 70)
